...
from app.dependencies.dependencies import get_df_handler, get_config_handler, get_sentence_scores, get_result_df_handler
from app.schemas.scores.score_values import ScoreValues
from app.services.df_mapper import DataFrameMapper
from app.services.df_processor import DataFrameProcessor
...
async def download_excel():
    # df = get_df_handler().get_dataframe()
    df = get_result_df_handler().get_dataframe()
...
async def get_all_sentences_short():
    df = get_result_df_handler().get_dataframe()
    scores = get_sentence_scores().get_scores()
    return DataFrameMapper.df_to_sentences_json(df, scores)

@data_router.get("/dataset/{sentence_id}")
async def get_special_sentence(sentence_id: int):
    df = get_result_df_handler().get_dataframe()
    scores = get_sentence_scores().get_scores()
    return DataFrameMapper.df_to_sentence_json(df, sentence_id, scores)
...
async def get_translations(
    ...
    scores.initialize(unique_sentences, target_languages, ScoreValues.NOT_EVALUATED)
    ...
    # df_handler.set_dataframe(new_df)
    #
